...
2. 変更内容を Git でコミットする（コミットメッセージにはタスク ID とタイトルを含める）
3. リモートリポジトリにプッシュする
4. 次のタスクを選択し Step 1 に戻る

#### Git コミット規約

```bash
# コミットメッセージ形式: [タスクID] タスクタイトル
git add -A
git commit -m "[E1-01] Vite + React + TypeScript プロジェクトのセットアップ"

# リモートにプッシュ
git push origin main
```

タスク完了ごとに必ずコミット・プッシュを行い、進捗をリモートリポジトリに反映させること。
